...
export const KPIHeader: React.FC<{ accounts: Account[] }> = ({ accounts }) => {
  ...
  const Card = ({ label, val, sub }: any) => (
    <div className="theme-bg-card border theme-border p-4 sm:p-6 md:p-10 lg:p-12 rounded shadow-2xl relative overflow-hidden group">
      <div className="absolute top-0 right-0 w-1 h-full bg-[#FF2D92] opacity-0 group-hover:opacity-100 transition-opacity" />
      <p className="text-[10px] md:text-sm theme-text-sub uppercase font-black tracking-[0.15em] md:tracking-[0.3em] mb-2 sm:mb-4 md:mb-6 opacity-40">{label}</p>
      <p className="text-xl sm:text-2xl md:text-5xl lg:text-6xl font-black theme-text-main tracking-tighter leading-none truncate">{val}</p>
      {sub && <p className="text-[10px] sm:text-xs md:text-base theme-text-sub mt-2 sm:mt-4 md:mt-6 font-black uppercase tracking-wider md:tracking-widest opacity-80">{sub}</p>}
    </div>
  );

  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-8 lg:gap-12 mb-8 md:mb-16 lg:mb-24">
